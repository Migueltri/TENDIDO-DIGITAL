...
const apiKey = ""; // Se inyecta automáticamente en tiempo de ejecución

// Helper: Llamada a Gemini para Texto
...
// Helper: Convertir PCM a WAV (Para el audio de Gemini)
...
// Helper: Llamada a Gemini para Audio (TTS)
...
function ArticleDetail({ article, onBack }) {
  ...
  const handleSummarize = async () => {
    ...
    const text = await generateText(`Resume esta noticia taurina en 3 puntos clave breves: ${article.fullContent || article.body}`);
    setSummary(text);
    setLoadingSummary(false);
  };

  // AI: Generar Audio (Radio)
  const handleTTS = async () => {
    setLoadingAudio(true);
    const script = await generateText(`Escribe un guion de radio muy breve (máximo 2 frases) para leer esta noticia en un boletín. Solo el texto: ${article.fullContent || article.body}`);
      ...
      <article className="bg-white rounded-xl shadow-xl overflow-hidden border border-stone-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 p-6 md:p-10">
          ...
          <div className="lg:col-span-8">
            ...
            {/* HERRAMIENTAS IA LECTOR */}
            <div className="flex gap-4 mb-6 pb-6 border-b border-stone-200">
                <button 
                    onClick={handleSummarize} 
                    disabled={loadingSummary || summary}
                    className="flex items-center gap-2 px-4 py-2 bg-yellow-100 text-yellow-900 rounded-lg hover:bg-yellow-200 transition text-sm font-bold border border-yellow-200"
                >
                    {loadingSummary ? "Analizando..." : <><Sparkles size={16} /> Resumen Rápido</>}
                </button>
                <button 
                    onClick={handleTTS} 
                    disabled={loadingAudio || audioUrl}
                    className="flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-900 rounded-lg hover:bg-blue-200 transition text-sm font-bold border border-blue-200"
                >
                    {loadingAudio ? "Generando audio..." : <><PlayCircle size={16} /> Escuchar Noticia</>}
                </button>
            </div>

            {summary && (
                <div className="bg-yellow-50 p-4 rounded-lg mb-6 border-l-4 border-yellow-500 text-stone-700 text-sm">
                    <h4 className="font-bold text-yellow-900 mb-2 flex items-center gap-2"><Sparkles size={14}/> Puntos Clave IA</h4>
            ...
            {audioUrl && (
                <div className="bg-blue-50 p-4 rounded-lg mb-6 border-l-4 border-blue-500">
                    <p className="text-xs font-bold text-blue-800 mb-2">📻 Boletín de Radio IA</p>
...
function AdminPanel({ user, articles }) {
  ...
  const handleGenerateTitle = async () => {
    if (!formData.fullContent) return alert("Escribe primero el contenido.");
    setGeneratingTitle(true);
    const text = await generateText(`Genera un titular periodístico corto, atractivo y serio para una noticia taurina basada en este texto. No uses comillas: ${formData.fullContent}`);
  ...
  const handleGenerateExcerpt = async () => {
    ...
    const text = await generateText(`Genera un resumen muy breve (máximo 2 frases) de este texto para usar como entradilla: ${formData.fullContent}`);
  ...
  const handleImproveContent = async () => {
    ...
    const text = await generateText(`Actúa como un redactor experto en tauromaquia. Reescribe este texto para que tenga un estilo periodístico profesional. Usa negritas markdown (**texto**) para resaltar nombres propios importantes (toreros, ganaderías, premios): ${formData.fullContent}`);
    setFormData(prev => ({...prev, fullContent: text}));
                ...
                <>
                    <div>
                        <div className="flex justify-between items-center mb-1">
                            ...
                            <button type="button" onClick={handleGenerateTitle} disabled={generatingTitle} className="text-xs flex items-center text-purple-600 hover:text-purple-800 font-bold bg-purple-50 px-2 py-1 rounded">
                                {generatingTitle ? "..." : <><Sparkles size={12} className="mr-1"/> Generar con IA</>}
                            </button>
                        </div>
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-1">
                            <label className="block text-xs font-bold text-stone-500 uppercase">Entradilla</label>
                            <button type="button" onClick={handleGenerateExcerpt} disabled={generatingExcerpt} className="text-xs flex items-center text-purple-600 hover:text-purple-800 font-bold bg-purple-50 px-2 py-1 rounded">
                                {generatingExcerpt ? "..." : <><Wand2 size={12} className="mr-1"/> Crear Resumen IA</>}
                            </button>
                        </div>
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-1">
                            <label className="block text-xs font-bold text-stone-500 uppercase">Cuerpo Completo</label>
                            <button type="button" onClick={handleImproveContent} disabled={improvingContent} className="text-xs flex items-center text-purple-600 hover:text-purple-800 font-bold bg-purple-50 px-2 py-1 rounded">
                                {improvingContent ? "Mejorando..." : <><FileText size={12} className="mr-1"/> Mejorar Redacción IA</>}
                            </button>
                        </div>
